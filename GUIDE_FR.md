...
**Ce qu'il faut en retenir concrètement :** la 1.29.0 retenue ici est postérieure au correctif, c'est donc la bonne version. Et il existe un **seuil à ne jamais franchir vers le bas : v1.27.0**. Avec une v1.26.x sur cette carte, le macropad s'énumère normalement, Windows affiche bien un clavier, et **aucune touche n'arrive jamais** — le symptôme le plus trompeur de tout le projet. Le test physique d'une lettre est obligatoire. **[CONFIRMÉ SUR LE MATÉRIEL]** Le test physique a été fait : sur cette carte, avec MicroPython v1.28.0 en variante standard, le macropad s'énumère en clavier HID sous Windows et **tape réellement** — un appui sur B1 en profil CIVIL3D produit `_MATCHPROP` + Entrée, avec le soulignement correctement placé en AZERTY. Le bug des rapports vides ne se manifeste pas dans cette configuration.

La réserve de méthode reste néanmoins valable pour quiconque reprendrait ce projet sur une autre carte ou une autre version : ce projet est une implémentation basée sur l'API officielle, et le test de la lettre unique doit toujours être fait en premier. Si l'interface s'énumère mais ne tape rien, arrêter à l'étape 7 et relever versions/ports/comportement ; ne pas inventer un correctif ni changer de langage en silence.
